...
# Return:
#   none
def print_end_game_status(players_scores, players_hands, dealer_hand):
  print_header('GAME RESULT')
  elimination = []
  for player,value in players_hands.items(): 
    if value <= 21 and (value > dealer_hand or dealer_hand > 21):
      players_scores[player] += 1
      print('{} wins! Score: {}'.format(player, players_scores[player]))
    elif value > 21 or (dealer_hand <= 21 and dealer_hand > value):
      players_scores[player] -= 1
      print('{} loses! Score: {}'.format(player, players_scores[player]))
      if players_scores[player] == 0:
        print('{} eliminated!'.format(player))
        players_scores.pop(player)
        elimination.append(player)
    else:
      print('{} pushes. Score: {}'.format(player, players_scores[player]))
  for player in elimination:
      players_hands.pop(player)
  if len(players_scores) == 0:
    print("All players eliminated!")


def continue_game(players_scores):
